...
if (!class_exists('ATAP_Class')) {
    ...
    class ATAP_Class {
        ...
        function plugin_init() {
            ...
            load_plugin_textdomain('atap', false, dirname(plugin_basename(__FILE__)) . '/languages');
        }
        ...
        function atap_admin_menu() {
            add_menu_page(__('AccessPress Twitter Auto Post', ATAP_TD), __('AccessPress Twitter Auto Post', ATAP_TD), 'manage_options', 'atap', array($this, 'plugin_settings'));
        }
        ...
        function atap_clear_log() {
            if (!empty($_GET) && wp_verify_nonce($_GET['_wpnonce'], 'atap-clear-log-nonce')) {
                ...
                $_SESSION['atap_message'] = __('Logs cleared successfully.', ATAP_TD);
        ...
        function delete_log() {
            if (!empty($_GET) && wp_verify_nonce($_GET['_wpnonce'], 'atap_delete_nonce')) {
                ...
                $_SESSION['atap_message'] = __('Log Deleted Successfully', ATAP_TD);
        ...
        function add_atap_meta_box($post_type) {
            add_meta_box(
                    'atap_meta_box'
                    , __('AccessPress Twitter Auto Post', ATAP_TD)
        ...
        function render_meta_box_content($post) {
            ...
            ?>
            <label for="atap_auto_post"><?php _e('Enable Auto Post', ATAP_TD); ?></label>
            <p>
                <select name="atap_auto_post">
                    <option value="yes" <?php selected($auto_post, 'yes'); ?>><?php _e('Yes', ATAP_TD); ?></option>
                    <option value="no" <?php selected($auto_post, 'no'); ?>><?php _e('No', ATAP_TD); ?></option>
                </select>
            </p>
            <?php
        }
        ...
        function restore_settings() {
            if(!empty($_GET) && wp_verify_nonce($_GET['_wpnonce'],'atap-restore-nonce')){
                $atap_settings = $this->get_default_settings();
                update_option('atap_settings', $atap_settings);
                $_SESSION['atap_message'] = __('Default Settings Restored Successfully', ATAP_TD);
